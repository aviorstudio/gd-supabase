...
MANIFEST = tuple(
    line.strip()
    for line in (ROOT / "package-manifest.txt").read_text(encoding="utf-8").splitlines()
    if line.strip()
)
...
def verify(archive: pathlib.Path) -> None:
    if not MANIFEST or len(MANIFEST) != len(set(MANIFEST)):
        raise ValueError("package manifest must be non-empty and unique")
...
def build(archive: pathlib.Path) -> None:
    symlinks = tuple(path for path in (ROOT / "addon").rglob("*") if path.is_symlink())
    if symlinks:
        raise ValueError(f"addon tree contains symlinks: {symlinks}")
    addon_files = tuple(
        sorted(
            path.relative_to(ROOT / "addon").as_posix()
            for path in (ROOT / "addon").rglob("*")
            if path.is_file()
        )
    )
    if addon_files != tuple(sorted(MANIFEST)):
        raise ValueError(f"addon tree differs from closed manifest: {addon_files}")
