...
- Existing plaintext legacy files are read only for an explicit migration into an injected credential adapter. Destination write and readback must both succeed, and this release retains the source even after success; it never destructively removes legacy data (D-08).
